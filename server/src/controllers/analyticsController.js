...
// Plan pricing in INR for revenue estimates
const PLAN_PRICE = { Free: 0, Monthly: 199, Yearly: 1499 };
// Initial tokens per plan
const PLAN_TOKENS = { Free: 50, Monthly: 300, Yearly: 1000 };
...
export const getPlatformAnalytics = async (req, res, next) => {
  try {
    ...
    const [
      ...
      User.find({ role: { $ne: 'superAdmin' } }).select('plan tokens role createdAt pendingCollege'),
    ...
    const calcRevenue = (users) =>
      users.reduce((sum, u) => sum + (PLAN_PRICE[u.plan] || 0), 0);
    ...
    const totalTokensConsumed = allUsers.reduce((sum, u) => {
      const initial = PLAN_TOKENS[u.plan] || 50;
